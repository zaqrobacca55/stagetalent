[index.html](https://github.com/user-attachments/files/28693935/index.html)
<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8">
  ...
  <div class="grid">
    ...
    <div>
      ...
      <div class="card" style="margin-top:22px;">
        ...
        <div class="row2">
          <div class="fld"><label>氏名（漢字）</label><input id="f_name" placeholder="愛原　翠"></div>
          <div class="fld"><label>氏名（ローマ字・自動）</label><input id="f_roma" placeholder="Aihara Midori"></div>
        </div>
        <div class="row2">
          <div class="fld"><label>かな姓（区切り確認用）</label><input id="f_kana_sei" placeholder="あいはら"></div>
          <div class="fld"><label>かな名</label><input id="f_kana_mei" placeholder="みどり"></div>
        </div>
        <div class="note" style="margin:-4px 0 12px;">※かなの姓名の区切りは自動推定です。違っていたら直すと、ローマ字氏名と画像slugに反映されます。</div>
        <div class="fld"><label>生年月日</label><input id="f_birth" placeholder="1980年11月13日"></div>
        <div class="row3">
          <div class="fld"><label>B</label><input id="f_b" placeholder="80cm"></div>
          <div class="fld"><label>W</label><input id="f_w" placeholder="65cm"></div>
          <div class="fld"><label>H</label><input id="f_h" placeholder="89cm"></div>
        </div>
        <div class="row2">
          <div class="fld"><label>身長</label><input id="f_height" placeholder="158cm"></div>
          <div class="fld"><label>靴のサイズ</label><input id="f_shoe" placeholder="23cm"></div>
        </div>
        <div class="fld"><label>出身地</label><input id="f_from" placeholder="東京都"></div>
        <div class="fld"><label>趣味（・区切り。改行は・に変換されます）</label><textarea id="f_hobby" placeholder="オーガニックカフェ・お店探し・ヨガ・旅行"></textarea></div>
        <div class="fld"><label>特技（・区切り）</label><input id="f_skill" placeholder="ヨガ・ウォーキング・英会話"></div>
        <div class="fld"><label>資格（・区切り。無ければ空欄でOK）</label><input id="f_license" placeholder="中型自動車免許・TOCFL B2"></div>
        <div class="fld">
          <label>出演情報（1行＝1項目。「区分: 内容」の形式。区分は CM / WEB / TV など）</label>
          <textarea id="f_credits" style="min-height:120px;" placeholder="CM: タクシーGO
WEB: オルビス"></textarea>
        </div>
        ...
        <div class="row3">
          ...
          <div class="fld"><label>ローマ字姓（小文字・画像用）</label><input id="i_slug" placeholder="aihara"></div>
        </div>
        <div class="fld"><label>生成されるベースURL（自動・編集可）</label><input id="i_preview" placeholder="https://stage-model.com/wp-content/uploads/2026/06/takagi"></div>
        ...
        <div class="fld"><label>alt属性テキスト（性別・年齢から自動／編集可）</label><input id="i_alt" placeholder="40代・50代の女性ミドルモデル・タレント　高木　えま"></div>
    ...
    <div>
      <div class="card">
        ...
        <div class="note">このコードはたかぎえまPDFを反映したテンプレートと同じ構造です。そのままCMSの本文に貼り付けられます。</div>
      </div>
    </div>
  </div>
</div>
...
function guessGenderByName(kanaMei, kanjiName){
  ...
  // ---- 男性寄りの手がかり ----
  // かな名末尾（男性に多い終わり）。「ま」は えま/ゆま 等で女性も多いため除外。
...
function romaToSlug(roma){
  // 「Ema Takagi」→ 姓(最後の語) takagi。1語なら全部を採用。
...
function buildHTML(){
  ...
  const creditDls = v("f_credits").split("\n").map(s=>s.trim()).filter(Boolean).map(line=>{
